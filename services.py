"""
NLP tasks: keyword extraction, document clustering, and corpus building.
"""
import numpy as np
import logging
...
def derive_top_keywords_hybrid(text: str, k: int, model, nlp) -> List[str]:
    ...
    filtered_cands = [c for c in base_cands if not any(n in c.lower() for n in noise) and len(c) < 60]
    ...
    for idx in np.argsort(-sims):
        ...
        if not current_tokens & seen_tokens:  # Avoid semantic duplicates by checking token overlap
...
def auto_k_by_silhouette(X: np.ndarray, k_min: int = 2, k_max: int = 10) -> int:
    ...
    for k in range(k_min, min(k_max, X.shape[0])):
        try:
            ...
            if len(set(labels)) > 1:
                score = silhouette_score(X, labels, metric="euclidean")  # Maximize cluster separation/cohesion
                if score > best_score:
...
def discover_profiles_from_upload(text: str, pages: int, per_page: int, seed_label: str, model, nlp) -> Tuple[List[Dict], Dict]:
    ...
    if not profiles and len(keywords) >= 2:
        ...
        fallback_kws = keywords[:min(3, max(2, len(keywords) // 3))]  # Use top 1/3 of keywords, max 3
        fallback_query = " ".join(fallback_kws)
        fallback_results = fetch_and_enrich_openalex(query_openalex(fallback_query, pages=pages, per_page=per_page))
...
def build_corpus_from_keywords(topic: str, pages: int, per_page: int, doaj_pages: int, progress_callback) -> List[Dict]:
    ...
    results = fetch_and_enrich_openalex(query_openalex(topic, pages=pages, per_page=per_page, progress=progress_callback), progress=progress_callback)
    profiles = openalex_to_profiles(results)
